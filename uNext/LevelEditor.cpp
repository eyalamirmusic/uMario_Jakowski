...
void LevelEditor::Update()
{
    ...
    {
        rBlock.x =
            CCore::mouseX - (-(int) CCore::getMap()->getXPos() + CCore::mouseX) % 32;
...
void LevelEditor::Draw(SDL_Renderer* rR)
{
    ...
    SDL_SetTextureAlphaMod(CCore::getMap()
                               ->getBlock(currentBlockID)
                               ->getSprite()
                               ->getTexture()
                               ->getIMG(),
                           225);
...
void LevelEditor::editMap()
{
    if (CCore::mouseLeftPressed)
    {
        CCore::getMap()
            ->getMapBlock(
                (int) (((-CCore::getMap()->getXPos() + CCore::mouseX)
                        - (int) ((-CCore::getMap()->getXPos() + CCore::mouseX)) % 32)
                       / 32),
                (CCFG::GAME_HEIGHT - CCore::mouseY + 16) / 32)
