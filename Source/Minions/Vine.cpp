...
void Vine::draw(SDL_Renderer* rR, CIMG* iIMG)
{
    ...
    if (jumpDistance < 32)
    {
        map->getMinionBlock(iBlockID - 1)
            ->Draw(rR,
                   (int) (iX * 32 + map->getXPos()),
    ...
    map->getBlock(map->getMapBlock((int) iX, (int) iY)->getBlockID())
        ->getSprite()
        ->getTexture()
        ->draw(rR,
               (int) (iX * 32 + map->getXPos()),
               (int) (getCFG().GAME_HEIGHT - iY * 32 - 16
                      - map->getMapBlock((int) iX, (int) iY)->getYPos()));
}
...
void Vine::collisionWithPlayer(bool TOP)
{
    if (minionState == 0)
    {
        ...
        if (fXPos + iHitBoxX / 2 > map->getPlayer()->getXPos() - map->getXPos())
        {
        ...
        {
            event->vOLDDir.push_back(Mario::Animations::Left);
            event->vOLDLength.push_back(map->getPlayer()->getHitBoxX() - 4);
        }

        for (int i = 0;
             i < map->getPlayer()->getYPos() + map->getPlayer()->getHitBoxY();
             i += 32)
        {
            event->vOLDDir.push_back(Mario::Animations::Vine1);
            event->vOLDLength.push_back(16);
            event->vOLDDir.push_back(Mario::Animations::Vine2);
            event->vOLDLength.push_back(16);
        }
        ...
        switch (map->getCurrentLevelID())
        {
            case 4:
            {
                ...
                event->vOLDDir.push_back(Mario::Animations::VineSpawn);
        ...
        {
            event->vNEWDir.push_back(Mario::Animations::Vine1);
            event->vNEWLength.push_back(16);
            event->vNEWDir.push_back(Mario::Animations::Vine2);
            event->vNEWLength.push_back(16);
        }
    }
    else {}
