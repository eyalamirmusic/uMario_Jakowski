...
void Map::update()
{
    ...
    for (int i = 0; i < lBlockDebris.size(); i++)
    {
        auto current = lBlockDebris[i];

        if (current->getDebrisState() != -1)
            current->update();
        else
        {
            delete current;
    ...
    for (int i = 0; i < lCoin.size(); i++)
    {
        auto current = lCoin[i];
        ...
        {
            lPoints.push_back(
                new Points(current->getXPos(), current->getYPos(), "200"));
            delete current;
...
void Map::updateMinions()
{
    ...
    for (int i = 0; i < lMinion.size(); i++)
    {
        for (int j = 0, jSize = lMinion[i].size(); j < jSize; j++)
        {
            ...
            if (current->minionSpawned)
            {
                if (current->minionState == -1)
                {
                    delete current;
                ...
                if (floor(current->fXPos / 160) != i)
                {
                    lMinion[(int) floor((int) current->fXPos / 160)].push_back(
                        current);
                    lMinion[i].erase(lMinion[i].begin() + j);
                    jSize = lMinion[i].size();
                }
...
{
    Mario::Collisions::updateMinionCollisions(lMinion);

    if (!inEvent && !oPlayer->getInLevelAnimation())
    {
        // ----- COLLISION WITH PLAYER
        for (int i = getListID(-(int) fXPos + oPlayer->getXPos())
                     - (getListID(-(int) fXPos + oPlayer->getXPos()) > 0 ? 1 : 0),
                 iSize = i + 2;
             i < iSize;
             i++)
        {
...
bool Map::checkCollisionRC(
    int nX, int nY, int nHitBoxX, int nHitBoxY, bool checkVisible)
{
    return checkCollision(getBlockID(nX + nHitBoxX, nY + nHitBoxY), checkVisible);
}

bool Map::checkCollisionRB(
    int nX, int nY, int nHitBoxX, int nHitBoxY, bool checkVisible)
{
    return checkCollision(getBlockID(nX + nHitBoxX, nY + nHitBoxY), checkVisible);
}
...
int Map::checkCollisionWithPlatform(int nX, int nY, int iHitBoxX, int iHitBoxY)
{
    for (unsigned int i = 0; i < vPlatform.size(); i++)
    {
        if (-fXPos + nX + iHitBoxX >= vPlatform[i]->getXPos()
            && -fXPos + nX <= vPlatform[i]->getXPos() + vPlatform[i]->getSize() * 16)
        {
            if (nY + iHitBoxY >= vPlatform[i]->getYPos()
                && nY + iHitBoxY <= vPlatform[i]->getYPos() + 16)
            {
                return i;
            }
        }
...
{
    bool output =
        vBlock[lMap[nV->getX()][nV->getY()]->getBlockID()]->getCollision()
        && (checkVisible
                ? vBlock[lMap[nV->getX()][nV->getY()]->getBlockID()]->getVisible()
                : true);
    delete nV;
    return output;
}
...
void Map::checkCollisionOnTopOfTheBlock(int nX, int nY)
{
         ...
         i++)
    {
        ...
        {
            if (!lMinion[i][j]->collisionOnlyWithPlayer
                && lMinion[i][j]->getMinionState() >= 0
                && ((lMinion[i][j]->getXPos() >= nX * 32
                     && lMinion[i][j]->getXPos() <= nX * 32 + 32)
                    || (lMinion[i][j]->getXPos() + lMinion[i][j]->iHitBoxX >= nX * 32
...
{
    lMinion[getListID(iX)].push_back(new Goombas(iX,
                                                 iY,
                                                 iLevelType == 0 || iLevelType == 4
                                                     ? 0
                                                 : iLevelType == 1 ? 8
                                                                   : 10,
                                                 moveDirection));
...
void Map::addKoppa(int iX, int iY, int minionState, bool moveDirection)
{
    ...
    switch (minionState)
    {
        case 0:
        case 3:
            tempBlock = iLevelType == 0 || iLevelType == 4 ? 7
                        : iLevelType == 1                  ? 14
                                                           : 17;
            break;
        case 1:
            tempBlock = iLevelType == 0 || iLevelType == 4 ? 4
                        : iLevelType == 1                  ? 12
                                                           : 15;
            break;
        case 2:
            tempBlock = iLevelType == 0 || iLevelType == 4 ? 5
                        : iLevelType == 1                  ? 13
                                                           : 16;
            break;
    }

    lMinion[getListID(iX)].push_back(
        new Koppa(iX, iY, minionState, moveDirection, tempBlock));
}
...
{
    lMinion[getListID(iX)].push_back(
        new Plant(iX, iY, iLevelType == 0 || iLevelType == 4 ? 18 : 19));
}
...
void Map::addFireBall(int X, int Y, int iWidth, int iSliceID, bool DIR)
{
    ...
    {
        lMinion[getListID((int) X)].push_back(
            new FireBall(X + 8, Y + 8, 14 * i, iSliceID, DIR));
    }
}
...
{
    lMinion[getListID(X)].push_back(
        new Cheep(X, Y, minionType, moveSpeed, moveDirection));
}
...
{
    lMinion[getListID(X)].push_back(
        new BulletBill(X, Y, moveDirection, minionState));
}
...
void Map::loadGameData(SDL_Renderer* rR)
{
    ...
    iDelay.push_back(0);
    vBlock.push_back(
        new Block(0, new Sprite(rR, tSprite, iDelay), false, true, false, false));
    tSprite.clear();
    iDelay.clear();
    // ----- 1 -----
    ...
    iDelay.push_back(0);
    vBlock.push_back(
        new Block(1, new Sprite(rR, tSprite, iDelay), true, false, true, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 2 -----
    ...
    vBlock.push_back(
        new Block(2, new Sprite(rR, tSprite, iDelay), true, false, true, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 3 -----
    ...
    iDelay.push_back(0);
    vBlock.push_back(
        new Block(3, new Sprite(rR, tSprite, iDelay), false, false, false, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 4 -----
    ...
    iDelay.push_back(0);
    vBlock.push_back(
        new Block(4, new Sprite(rR, tSprite, iDelay), false, false, false, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 5 -----
    ...
    iDelay.push_back(0);
    vBlock.push_back(
        new Block(5, new Sprite(rR, tSprite, iDelay), false, false, false, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 6 -----
    ...
    iDelay.push_back(0);
    vBlock.push_back(
        new Block(6, new Sprite(rR, tSprite, iDelay), false, false, false, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 7 -----
    ...
    iDelay.push_back(0);
    vBlock.push_back(
        new Block(7, new Sprite(rR, tSprite, iDelay), false, false, false, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 8 -----
    ...
    iDelay.push_back(140);
    vBlock.push_back(
        new Block(8, new Sprite(rR, tSprite, iDelay), true, false, true, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 9 -----
    ...
    iDelay.push_back(0);
    vBlock.push_back(
        new Block(9, new Sprite(rR, tSprite, iDelay), true, false, false, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 10 -----
    ...
    iDelay.push_back(0);
    vBlock.push_back(
        new Block(10, new Sprite(rR, tSprite, iDelay), false, false, false, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 11 -----
    ...
    iDelay.push_back(0);
    vBlock.push_back(
        new Block(11, new Sprite(rR, tSprite, iDelay), false, false, false, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 12 -----
    ...
    iDelay.push_back(0);
    vBlock.push_back(
        new Block(12, new Sprite(rR, tSprite, iDelay), false, false, false, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 13 -----
    ...
    iDelay.push_back(0);
    vBlock.push_back(
        new Block(13, new Sprite(rR, tSprite, iDelay), true, false, true, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 14 -----
    ...
    iDelay.push_back(0);
    vBlock.push_back(
        new Block(14, new Sprite(rR, tSprite, iDelay), false, false, false, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 15 -----
    ...
    iDelay.push_back(0);
    vBlock.push_back(
        new Block(15, new Sprite(rR, tSprite, iDelay), false, false, false, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 16 -----
    ...
    iDelay.push_back(0);
    vBlock.push_back(
        new Block(16, new Sprite(rR, tSprite, iDelay), false, false, false, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 17 -----
    ...
    iDelay.push_back(0);
    vBlock.push_back(
        new Block(17, new Sprite(rR, tSprite, iDelay), false, false, false, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 18 -----
    ...
    iDelay.push_back(0);
    vBlock.push_back(
        new Block(18, new Sprite(rR, tSprite, iDelay), false, false, false, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 19 -----
    ...
    iDelay.push_back(0);
    vBlock.push_back(
        new Block(19, new Sprite(rR, tSprite, iDelay), false, false, false, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 20 -----
    ...
    iDelay.push_back(0);
    vBlock.push_back(
        new Block(20, new Sprite(rR, tSprite, iDelay), true, false, false, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 21 -----
    ...
    iDelay.push_back(0);
    vBlock.push_back(
        new Block(21, new Sprite(rR, tSprite, iDelay), true, false, true, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 22 -----
    ...
    iDelay.push_back(0);
    vBlock.push_back(
        new Block(22, new Sprite(rR, tSprite, iDelay), true, false, false, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 23 -----
    ...
    iDelay.push_back(0);
    vBlock.push_back(
        new Block(23, new Sprite(rR, tSprite, iDelay), true, false, true, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 24 BlockQ2 -----
    tSprite.push_back("transp");
    iDelay.push_back(0);
    vBlock.push_back(
        new Block(24, new Sprite(rR, tSprite, iDelay), true, false, true, false));
    tSprite.clear();
    iDelay.clear();
    // ----- 25 -----
    ...
    iDelay.push_back(0);
    vBlock.push_back(
        new Block(25, new Sprite(rR, tSprite, iDelay), true, false, false, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 26 -----
    ...
    iDelay.push_back(0);
    vBlock.push_back(
        new Block(26, new Sprite(rR, tSprite, iDelay), true, false, false, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 27 -----
    ...
    iDelay.push_back(0);
    vBlock.push_back(
        new Block(27, new Sprite(rR, tSprite, iDelay), true, false, false, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 28 -----
    ...
    iDelay.push_back(0);
    vBlock.push_back(
        new Block(28, new Sprite(rR, tSprite, iDelay), true, false, true, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 29 -----
    ...
    iDelay.push_back(140);
    vBlock.push_back(
        new Block(29, new Sprite(rR, tSprite, iDelay), false, false, true, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 30 -----
    ...
    iDelay.push_back(0);
    vBlock.push_back(
        new Block(30, new Sprite(rR, tSprite, iDelay), true, false, false, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 31 -----
    ...
    iDelay.push_back(0);
    vBlock.push_back(
        new Block(31, new Sprite(rR, tSprite, iDelay), true, false, true, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 32 -----
    ...
    iDelay.push_back(0);
    vBlock.push_back(
        new Block(32, new Sprite(rR, tSprite, iDelay), true, false, false, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 33 -----
    ...
    iDelay.push_back(0);
    vBlock.push_back(
        new Block(33, new Sprite(rR, tSprite, iDelay), true, false, true, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 34 -----
    ...
    iDelay.push_back(0);
    vBlock.push_back(
        new Block(34, new Sprite(rR, tSprite, iDelay), true, false, false, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 35 -----
    ...
    iDelay.push_back(0);
    vBlock.push_back(
        new Block(35, new Sprite(rR, tSprite, iDelay), true, false, false, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 36 -----
    ...
    iDelay.push_back(0);
    vBlock.push_back(
        new Block(36, new Sprite(rR, tSprite, iDelay), true, false, true, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 37 -----
    ...
    iDelay.push_back(0);
    vBlock.push_back(
        new Block(37, new Sprite(rR, tSprite, iDelay), true, false, false, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 38 -----
    ...
    iDelay.push_back(0);
    vBlock.push_back(
        new Block(38, new Sprite(rR, tSprite, iDelay), true, false, true, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 39 -----
    ...
    iDelay.push_back(0);
    vBlock.push_back(
        new Block(39, new Sprite(rR, tSprite, iDelay), true, false, false, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 40 -----
    ...
    iDelay.push_back(0);
    vBlock.push_back(
        new Block(40, new Sprite(rR, tSprite, iDelay), false, false, true, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 41 -----
    ...
    iDelay.push_back(0);
    vBlock.push_back(
        new Block(41, new Sprite(rR, tSprite, iDelay), false, false, true, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 42 -----
    ...
    iDelay.push_back(0);
    vBlock.push_back(
        new Block(42, new Sprite(rR, tSprite, iDelay), false, false, false, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 43 -----
    ...
    iDelay.push_back(0);
    vBlock.push_back(
        new Block(43, new Sprite(rR, tSprite, iDelay), false, false, false, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 44 -----
    ...
    iDelay.push_back(0);
    vBlock.push_back(
        new Block(44, new Sprite(rR, tSprite, iDelay), false, false, false, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 45 -----
    ...
    iDelay.push_back(0);
    vBlock.push_back(
        new Block(45, new Sprite(rR, tSprite, iDelay), false, false, false, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 46 -----
    ...
    iDelay.push_back(0);
    vBlock.push_back(
        new Block(46, new Sprite(rR, tSprite, iDelay), false, false, false, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 47 -----
    ...
    iDelay.push_back(0);
    vBlock.push_back(
        new Block(47, new Sprite(rR, tSprite, iDelay), false, false, false, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 48 -----
    ...
    iDelay.push_back(0);
    vBlock.push_back(
        new Block(48, new Sprite(rR, tSprite, iDelay), false, false, false, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 49 -----
    ...
    iDelay.push_back(0);
    vBlock.push_back(
        new Block(49, new Sprite(rR, tSprite, iDelay), false, false, false, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 50 -----
    ...
    iDelay.push_back(0);
    vBlock.push_back(
        new Block(50, new Sprite(rR, tSprite, iDelay), false, false, false, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 51 -----
    ...
    iDelay.push_back(0);
    vBlock.push_back(
        new Block(51, new Sprite(rR, tSprite, iDelay), false, false, false, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 52 -----
    ...
    iDelay.push_back(0);
    vBlock.push_back(
        new Block(52, new Sprite(rR, tSprite, iDelay), false, false, false, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 53 -----
    ...
    iDelay.push_back(0);
    vBlock.push_back(
        new Block(53, new Sprite(rR, tSprite, iDelay), false, false, false, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 54 -----
    ...
    iDelay.push_back(0);
    vBlock.push_back(
        new Block(54, new Sprite(rR, tSprite, iDelay), false, false, false, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 55 -----
    ...
    iDelay.push_back(140);
    vBlock.push_back(
        new Block(55, new Sprite(rR, tSprite, iDelay), true, false, true, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 56 -----
    ...
    iDelay.push_back(0);
    vBlock.push_back(
        new Block(56, new Sprite(rR, tSprite, iDelay), true, false, false, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 57 -----
    ...
    iDelay.push_back(140);
    vBlock.push_back(
        new Block(57, new Sprite(rR, tSprite, iDelay), true, false, true, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 58 -----
    ...
    iDelay.push_back(0);
    vBlock.push_back(
        new Block(58, new Sprite(rR, tSprite, iDelay), true, false, false, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 59 -----
    ...
    iDelay.push_back(0);
    vBlock.push_back(
        new Block(59, new Sprite(rR, tSprite, iDelay), true, false, false, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 60 -----
    ...
    iDelay.push_back(0);
    vBlock.push_back(
        new Block(60, new Sprite(rR, tSprite, iDelay), true, false, true, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 61 -----
    ...
    iDelay.push_back(0);
    vBlock.push_back(
        new Block(61, new Sprite(rR, tSprite, iDelay), true, false, false, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 62 -----
    ...
    iDelay.push_back(0);
    vBlock.push_back(
        new Block(62, new Sprite(rR, tSprite, iDelay), true, false, true, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 63 -----
    ...
    iDelay.push_back(0);
    vBlock.push_back(
        new Block(63, new Sprite(rR, tSprite, iDelay), true, false, false, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 64 -----
    ...
    iDelay.push_back(0);
    vBlock.push_back(
        new Block(64, new Sprite(rR, tSprite, iDelay), false, false, false, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 65 -----
    ...
    iDelay.push_back(0);
    vBlock.push_back(
        new Block(65, new Sprite(rR, tSprite, iDelay), false, false, false, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 66 -----
    ...
    iDelay.push_back(0);
    vBlock.push_back(
        new Block(66, new Sprite(rR, tSprite, iDelay), false, false, false, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 67 -----
    ...
    iDelay.push_back(0);
    vBlock.push_back(
        new Block(67, new Sprite(rR, tSprite, iDelay), true, false, false, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 68 -----
    ...
    iDelay.push_back(0);
    vBlock.push_back(
        new Block(68, new Sprite(rR, tSprite, iDelay), true, false, false, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 69 -----
    ...
    iDelay.push_back(0);
    vBlock.push_back(
        new Block(69, new Sprite(rR, tSprite, iDelay), true, false, false, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 70 -----
    ...
    iDelay.push_back(0);
    vBlock.push_back(
        new Block(70, new Sprite(rR, tSprite, iDelay), false, false, false, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 71 -----
    ...
    iDelay.push_back(140);
    vBlock.push_back(
        new Block(71, new Sprite(rR, tSprite, iDelay), false, false, true, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 72 -----
    ...
    iDelay.push_back(140);
    vBlock.push_back(
        new Block(72, new Sprite(rR, tSprite, iDelay), false, false, true, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 73 -----
    ...
    iDelay.push_back(140);
    vBlock.push_back(
        new Block(73, new Sprite(rR, tSprite, iDelay), false, false, true, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 74 -----
    ...
    iDelay.push_back(0);
    vBlock.push_back(
        new Block(74, new Sprite(rR, tSprite, iDelay), true, false, false, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 75 -----
    ...
    iDelay.push_back(0);
    vBlock.push_back(
        new Block(75, new Sprite(rR, tSprite, iDelay), true, false, true, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 76 -----
    ...
    iDelay.push_back(0);
    vBlock.push_back(
        new Block(76, new Sprite(rR, tSprite, iDelay), true, false, true, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 77 -----
    ...
    iDelay.push_back(0);
    vBlock.push_back(
        new Block(77, new Sprite(rR, tSprite, iDelay), false, true, false, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 78 -----
    ...
    iDelay.push_back(0);
    vBlock.push_back(
        new Block(78, new Sprite(rR, tSprite, iDelay), false, true, false, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 79 -----
    ...
    iDelay.push_back(0);
    vBlock.push_back(
        new Block(78, new Sprite(rR, tSprite, iDelay), false, false, false, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 80 -----
    ...
    iDelay.push_back(0);
    vBlock.push_back(
        new Block(80, new Sprite(rR, tSprite, iDelay), true, false, false, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 81 -----
    ...
    iDelay.push_back(0);
    vBlock.push_back(
        new Block(81, new Sprite(rR, tSprite, iDelay), true, false, true, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 82 -----
    ...
    vBlock.push_back(
        new Block(82, new Sprite(rR, tSprite, iDelay), true, false, true, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 83 ----- END BRIDGE
    tSprite.push_back("transp");
    iDelay.push_back(0);
    vBlock.push_back(
        new Block(83, new Sprite(rR, tSprite, iDelay), true, false, false, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 84 -----
    ...
    iDelay.push_back(0);
    vBlock.push_back(
        new Block(84, new Sprite(rR, tSprite, iDelay), false, false, false, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 85 -----
    ...
    iDelay.push_back(0);
    vBlock.push_back(
        new Block(85, new Sprite(rR, tSprite, iDelay), false, false, false, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 86 -----
    ...
    iDelay.push_back(0);
    vBlock.push_back(
        new Block(86, new Sprite(rR, tSprite, iDelay), false, false, false, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 87 -----
    ...
    iDelay.push_back(0);
    vBlock.push_back(
        new Block(87, new Sprite(rR, tSprite, iDelay), false, false, false, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 88 -----
    ...
    iDelay.push_back(0);
    vBlock.push_back(
        new Block(88, new Sprite(rR, tSprite, iDelay), false, false, false, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 89 -----
    ...
    iDelay.push_back(0);
    vBlock.push_back(
        new Block(89, new Sprite(rR, tSprite, iDelay), false, false, false, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 90 -----
    ...
    iDelay.push_back(0);
    vBlock.push_back(
        new Block(90, new Sprite(rR, tSprite, iDelay), false, false, false, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 91 -----
    ...
    iDelay.push_back(0);
    vBlock.push_back(
        new Block(91, new Sprite(rR, tSprite, iDelay), false, false, false, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 92 -----
    ...
    iDelay.push_back(0);
    vBlock.push_back(
        new Block(92, new Sprite(rR, tSprite, iDelay), true, false, true, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 93 -----
    ...
    iDelay.push_back(0);
    vBlock.push_back(
        new Block(93, new Sprite(rR, tSprite, iDelay), true, false, true, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 94 -----
    ...
    iDelay.push_back(0);
    vBlock.push_back(
        new Block(94, new Sprite(rR, tSprite, iDelay), false, false, false, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 95 -----
    ...
    iDelay.push_back(0);
    vBlock.push_back(
        new Block(95, new Sprite(rR, tSprite, iDelay), false, false, false, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 96 -----
    ...
    iDelay.push_back(0);
    vBlock.push_back(
        new Block(96, new Sprite(rR, tSprite, iDelay), false, false, false, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 97 -----
    ...
    iDelay.push_back(0);
    vBlock.push_back(
        new Block(97, new Sprite(rR, tSprite, iDelay), true, false, false, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 98 -----
    ...
    iDelay.push_back(0);
    vBlock.push_back(
        new Block(98, new Sprite(rR, tSprite, iDelay), true, false, true, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 99 -----
    ...
    iDelay.push_back(0);
    vBlock.push_back(
        new Block(99, new Sprite(rR, tSprite, iDelay), true, false, false, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 100 -----
    ...
    iDelay.push_back(0);
    vBlock.push_back(
        new Block(100, new Sprite(rR, tSprite, iDelay), true, false, true, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 101 -----
    ...
    iDelay.push_back(0);
    vBlock.push_back(
        new Block(101, new Sprite(rR, tSprite, iDelay), true, false, false, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 102 -----
    ...
    iDelay.push_back(0);
    vBlock.push_back(
        new Block(102, new Sprite(rR, tSprite, iDelay), true, false, false, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 103 -----
    ...
    iDelay.push_back(0);
    vBlock.push_back(
        new Block(103, new Sprite(rR, tSprite, iDelay), true, false, true, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 104 -----
    ...
    iDelay.push_back(0);
    vBlock.push_back(
        new Block(104, new Sprite(rR, tSprite, iDelay), true, false, false, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 105 -----
    ...
    iDelay.push_back(0);
    vBlock.push_back(
        new Block(105, new Sprite(rR, tSprite, iDelay), true, false, true, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 106 -----
    ...
    iDelay.push_back(0);
    vBlock.push_back(
        new Block(106, new Sprite(rR, tSprite, iDelay), true, false, false, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 107 -----
    ...
    iDelay.push_back(0);
    vBlock.push_back(
        new Block(107, new Sprite(rR, tSprite, iDelay), true, false, false, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 108 -----
    ...
    iDelay.push_back(0);
    vBlock.push_back(
        new Block(108, new Sprite(rR, tSprite, iDelay), false, false, false, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 109 -----
    ...
    iDelay.push_back(0);
    vBlock.push_back(
        new Block(109, new Sprite(rR, tSprite, iDelay), false, false, false, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 110 -----
    ...
    iDelay.push_back(0);
    vBlock.push_back(
        new Block(110, new Sprite(rR, tSprite, iDelay), false, false, false, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 111 -----
    ...
    iDelay.push_back(0);
    vBlock.push_back(
        new Block(111, new Sprite(rR, tSprite, iDelay), false, false, false, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 112 -----
    ...
    iDelay.push_back(0);
    vBlock.push_back(
        new Block(112, new Sprite(rR, tSprite, iDelay), true, false, false, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 113 -----
    ...
    iDelay.push_back(0);
    vBlock.push_back(
        new Block(113, new Sprite(rR, tSprite, iDelay), true, false, true, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 114 -----
    ...
    iDelay.push_back(0);
    vBlock.push_back(
        new Block(114, new Sprite(rR, tSprite, iDelay), true, false, false, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 115 -----
    ...
    iDelay.push_back(0);
    vBlock.push_back(
        new Block(115, new Sprite(rR, tSprite, iDelay), true, false, true, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 116 -----
    ...
    iDelay.push_back(0);
    vBlock.push_back(
        new Block(116, new Sprite(rR, tSprite, iDelay), true, false, false, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 117 -----
    ...
    iDelay.push_back(0);
    vBlock.push_back(
        new Block(117, new Sprite(rR, tSprite, iDelay), true, false, false, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 118 -----
    ...
    iDelay.push_back(0);
    vBlock.push_back(
        new Block(118, new Sprite(rR, tSprite, iDelay), true, false, true, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 119 -----
    ...
    iDelay.push_back(0);
    vBlock.push_back(
        new Block(119, new Sprite(rR, tSprite, iDelay), true, false, false, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 120 -----
    ...
    iDelay.push_back(0);
    vBlock.push_back(
        new Block(120, new Sprite(rR, tSprite, iDelay), true, false, true, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 121 -----
    ...
    iDelay.push_back(0);
    vBlock.push_back(
        new Block(121, new Sprite(rR, tSprite, iDelay), true, false, false, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 122 -----
    ...
    iDelay.push_back(0);
    vBlock.push_back(
        new Block(122, new Sprite(rR, tSprite, iDelay), false, false, false, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 123 -----
    ...
    iDelay.push_back(0);
    vBlock.push_back(
        new Block(123, new Sprite(rR, tSprite, iDelay), false, false, true, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 124 -----
    ...
    iDelay.push_back(0);
    vBlock.push_back(
        new Block(124, new Sprite(rR, tSprite, iDelay), false, false, true, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 125 -----
    ...
    iDelay.push_back(0);
    vBlock.push_back(
        new Block(125, new Sprite(rR, tSprite, iDelay), true, false, true, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 126 -----
    ...
    iDelay.push_back(0);
    vBlock.push_back(
        new Block(126, new Sprite(rR, tSprite, iDelay), true, false, false, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 127 ----- // -- BONUS END
    tSprite.push_back("transp");
    iDelay.push_back(0);
    vBlock.push_back(
        new Block(127, new Sprite(rR, tSprite, iDelay), true, false, true, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 128 ----- // -- SPAWN VINE
    tSprite.push_back("brickred");
    iDelay.push_back(0);
    vBlock.push_back(
        new Block(128, new Sprite(rR, tSprite, iDelay), true, false, true, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 129 ----- // -- SPAWN VINE
    tSprite.push_back("brick1");
    iDelay.push_back(0);
    vBlock.push_back(
        new Block(129, new Sprite(rR, tSprite, iDelay), true, false, true, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 130 -----
    ...
    iDelay.push_back(0);
    vBlock.push_back(
        new Block(130, new Sprite(rR, tSprite, iDelay), true, false, false, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 131 -----
    ...
    iDelay.push_back(0);
    vBlock.push_back(
        new Block(131, new Sprite(rR, tSprite, iDelay), true, false, false, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 132 -----
    ...
    iDelay.push_back(0);
    vBlock.push_back(
        new Block(132, new Sprite(rR, tSprite, iDelay), false, false, false, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 133 -----
    ...
    iDelay.push_back(0);
    vBlock.push_back(
        new Block(133, new Sprite(rR, tSprite, iDelay), false, false, false, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 134 -----
    ...
    iDelay.push_back(0);
    vBlock.push_back(
        new Block(134, new Sprite(rR, tSprite, iDelay), false, false, false, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 135 -----
    ...
    iDelay.push_back(0);
    vBlock.push_back(
        new Block(135, new Sprite(rR, tSprite, iDelay), false, false, false, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 136 -----
    ...
    iDelay.push_back(0);
    vBlock.push_back(
        new Block(136, new Sprite(rR, tSprite, iDelay), true, false, false, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 137 -----
    ...
    iDelay.push_back(0);
    vBlock.push_back(
        new Block(137, new Sprite(rR, tSprite, iDelay), true, false, true, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 138 -----
    ...
    iDelay.push_back(0);
    vBlock.push_back(
        new Block(138, new Sprite(rR, tSprite, iDelay), true, false, false, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 139 -----
    ...
    iDelay.push_back(0);
    vBlock.push_back(
        new Block(139, new Sprite(rR, tSprite, iDelay), true, false, true, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 140 -----
    ...
    iDelay.push_back(0);
    vBlock.push_back(
        new Block(140, new Sprite(rR, tSprite, iDelay), true, false, false, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 141 -----
    ...
    iDelay.push_back(0);
    vBlock.push_back(
        new Block(141, new Sprite(rR, tSprite, iDelay), true, false, false, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 142 -----
    ...
    iDelay.push_back(0);
    vBlock.push_back(
        new Block(142, new Sprite(rR, tSprite, iDelay), true, false, false, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 143 -----
    ...
    iDelay.push_back(0);
    vBlock.push_back(
        new Block(143, new Sprite(rR, tSprite, iDelay), false, false, false, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 144 -----
    ...
    iDelay.push_back(0);
    vBlock.push_back(
        new Block(144, new Sprite(rR, tSprite, iDelay), false, false, false, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 145 -----
    ...
    iDelay.push_back(0);
    vBlock.push_back(
        new Block(145, new Sprite(rR, tSprite, iDelay), true, false, false, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 146 -----
    ...
    iDelay.push_back(0);
    vBlock.push_back(
        new Block(146, new Sprite(rR, tSprite, iDelay), true, false, false, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 147 -----
    ...
    iDelay.push_back(0);
    vBlock.push_back(
        new Block(147, new Sprite(rR, tSprite, iDelay), true, false, false, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 148 -----
    ...
    iDelay.push_back(0);
    vBlock.push_back(
        new Block(148, new Sprite(rR, tSprite, iDelay), false, false, false, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 149 -----
    ...
    iDelay.push_back(0);
    vBlock.push_back(
        new Block(149, new Sprite(rR, tSprite, iDelay), false, false, false, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 150 -----
    ...
    iDelay.push_back(0);
    vBlock.push_back(
        new Block(150, new Sprite(rR, tSprite, iDelay), false, false, false, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 151 -----
    ...
    iDelay.push_back(0);
    vBlock.push_back(
        new Block(151, new Sprite(rR, tSprite, iDelay), true, false, false, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 152 -----
    ...
    iDelay.push_back(0);
    vBlock.push_back(
        new Block(152, new Sprite(rR, tSprite, iDelay), true, false, false, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 153 -----
    ...
    iDelay.push_back(0);
    vBlock.push_back(
        new Block(153, new Sprite(rR, tSprite, iDelay), true, false, false, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 154 -----
    ...
    iDelay.push_back(0);
    vBlock.push_back(
        new Block(154, new Sprite(rR, tSprite, iDelay), false, false, false, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 155 -----
    ...
    iDelay.push_back(0);
    vBlock.push_back(
        new Block(155, new Sprite(rR, tSprite, iDelay), false, false, false, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 156 -----
    ...
    iDelay.push_back(0);
    vBlock.push_back(
        new Block(156, new Sprite(rR, tSprite, iDelay), false, false, false, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 157 -----
    ...
    iDelay.push_back(0);
    vBlock.push_back(
        new Block(157, new Sprite(rR, tSprite, iDelay), false, false, false, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 158 -----
    ...
    iDelay.push_back(0);
    vBlock.push_back(
        new Block(158, new Sprite(rR, tSprite, iDelay), false, false, false, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 159 -----
    ...
    iDelay.push_back(0);
    vBlock.push_back(
        new Block(159, new Sprite(rR, tSprite, iDelay), false, false, false, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 160 -----
    ...
    iDelay.push_back(0);
    vBlock.push_back(
        new Block(160, new Sprite(rR, tSprite, iDelay), false, false, false, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 161 -----
    ...
    iDelay.push_back(0);
    vBlock.push_back(
        new Block(161, new Sprite(rR, tSprite, iDelay), false, false, false, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 162 -----
    ...
    iDelay.push_back(0);
    vBlock.push_back(
        new Block(162, new Sprite(rR, tSprite, iDelay), false, false, false, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 163 -----
    ...
    iDelay.push_back(0);
    vBlock.push_back(
        new Block(163, new Sprite(rR, tSprite, iDelay), false, false, false, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 164 -----
    ...
    iDelay.push_back(0);
    vBlock.push_back(
        new Block(164, new Sprite(rR, tSprite, iDelay), false, false, false, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 165 -----
    ...
    iDelay.push_back(0);
    vBlock.push_back(
        new Block(165, new Sprite(rR, tSprite, iDelay), false, false, false, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 166 -----
    ...
    iDelay.push_back(0);
    vBlock.push_back(
        new Block(166, new Sprite(rR, tSprite, iDelay), true, false, false, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 167 -----
    ...
    iDelay.push_back(0);
    vBlock.push_back(
        new Block(167, new Sprite(rR, tSprite, iDelay), true, false, false, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 168 -----
    ...
    iDelay.push_back(0);
    vBlock.push_back(
        new Block(168, new Sprite(rR, tSprite, iDelay), false, false, false, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 169 ----- TP
    tSprite.push_back("transp");
    iDelay.push_back(0);
    vBlock.push_back(
        new Block(169, new Sprite(rR, tSprite, iDelay), false, false, true, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 170 ----- TP2
    tSprite.push_back("transp");
    iDelay.push_back(0);
    vBlock.push_back(
        new Block(170, new Sprite(rR, tSprite, iDelay), false, false, true, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 171 ----- TP3 - bTP
    tSprite.push_back("transp");
    iDelay.push_back(0);
    vBlock.push_back(
        new Block(171, new Sprite(rR, tSprite, iDelay), false, false, true, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 172 -----
    ...
    iDelay.push_back(0);
    vBlock.push_back(
        new Block(172, new Sprite(rR, tSprite, iDelay), true, false, false, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 173 -----
    ...
    iDelay.push_back(0);
    vBlock.push_back(
        new Block(173, new Sprite(rR, tSprite, iDelay), true, false, true, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 174 -----
    ...
    iDelay.push_back(0);
    vBlock.push_back(
        new Block(174, new Sprite(rR, tSprite, iDelay), true, false, false, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 175 -----
    ...
    iDelay.push_back(0);
    vBlock.push_back(
        new Block(175, new Sprite(rR, tSprite, iDelay), true, false, true, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 176 -----
    ...
    iDelay.push_back(0);
    vBlock.push_back(
        new Block(176, new Sprite(rR, tSprite, iDelay), true, false, false, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 177 -----
    ...
    iDelay.push_back(0);
    vBlock.push_back(
        new Block(177, new Sprite(rR, tSprite, iDelay), true, false, true, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 178 -----
    ...
    iDelay.push_back(0);
    vBlock.push_back(
        new Block(178, new Sprite(rR, tSprite, iDelay), true, false, false, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 179 -----
    ...
    iDelay.push_back(0);
    vBlock.push_back(
        new Block(179, new Sprite(rR, tSprite, iDelay), true, false, true, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 180 -----
    ...
    iDelay.push_back(0);
    vBlock.push_back(
        new Block(180, new Sprite(rR, tSprite, iDelay), false, false, false, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 181 -----
    ...
    iDelay.push_back(0);
    vBlock.push_back(
        new Block(182, new Sprite(rR, tSprite, iDelay), false, false, true, true));
    tSprite.clear();
    iDelay.clear();
    ...
    iDelay.push_back(200);
    vMinion.push_back(
        new Block(0, new Sprite(rR, tSprite, iDelay), true, false, true, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 1 -----
    ...
    iDelay.push_back(0);
    vMinion.push_back(
        new Block(3, new Sprite(rR, tSprite, iDelay), true, false, true, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 4 -----
    ...
    iDelay.push_back(200);
    vMinion.push_back(
        new Block(4, new Sprite(rR, tSprite, iDelay), true, false, true, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 5 -----
    ...
    iDelay.push_back(0);
    vMinion.push_back(
        new Block(5, new Sprite(rR, tSprite, iDelay), true, false, true, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 6 -----
    ...
    iDelay.push_back(50);
    vMinion.push_back(
        new Block(6, new Sprite(rR, tSprite, iDelay), true, false, true, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 7 -----
    ...
    iDelay.push_back(200);
    vMinion.push_back(
        new Block(7, new Sprite(rR, tSprite, iDelay), true, false, true, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 8 -----
    ...
    iDelay.push_back(0);
    vMinion.push_back(
        new Block(9, new Sprite(rR, tSprite, iDelay), true, false, true, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 10 -----
    ...
    iDelay.push_back(200);
    vMinion.push_back(
        new Block(10, new Sprite(rR, tSprite, iDelay), true, false, true, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 11 -----
    ...
    iDelay.push_back(0);
    vMinion.push_back(
        new Block(11, new Sprite(rR, tSprite, iDelay), true, false, true, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 12 -----
    ...
    iDelay.push_back(200);
    vMinion.push_back(
        new Block(12, new Sprite(rR, tSprite, iDelay), true, false, true, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 13 -----
    ...
    iDelay.push_back(200);
    vMinion.push_back(
        new Block(14, new Sprite(rR, tSprite, iDelay), true, false, true, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 15 -----
    ...
    iDelay.push_back(200);
    vMinion.push_back(
        new Block(15, new Sprite(rR, tSprite, iDelay), true, false, true, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 16 -----
    ...
    iDelay.push_back(0);
    vMinion.push_back(
        new Block(16, new Sprite(rR, tSprite, iDelay), true, false, true, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 17 -----
    ...
    iDelay.push_back(200);
    vMinion.push_back(
        new Block(17, new Sprite(rR, tSprite, iDelay), true, false, true, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 18 -----
    ...
    iDelay.push_back(125);
    vMinion.push_back(
        new Block(18, new Sprite(rR, tSprite, iDelay), true, false, true, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 19 -----
    ...
    iDelay.push_back(125);
    vMinion.push_back(
        new Block(19, new Sprite(rR, tSprite, iDelay), true, false, true, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 20 -----
    ...
    iDelay.push_back(285);
    vMinion.push_back(
        new Block(20, new Sprite(rR, tSprite, iDelay), true, false, true, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 21 -----
    ...
    iDelay.push_back(75);
    vMinion.push_back(
        new Block(23, new Sprite(rR, tSprite, iDelay), true, false, false, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 24 -----
    ...
    iDelay.push_back(75);
    vMinion.push_back(
        new Block(24, new Sprite(rR, tSprite, iDelay), true, false, false, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 25 -----
    ...
    iDelay.push_back(0);
    vMinion.push_back(
        new Block(25, new Sprite(rR, tSprite, iDelay), true, false, true, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 26 -----
    ...
    iDelay.push_back(0);
    vMinion.push_back(
        new Block(26, new Sprite(rR, tSprite, iDelay), false, false, false, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 27 -----
    ...
    iDelay.push_back(0);
    vMinion.push_back(
        new Block(27, new Sprite(rR, tSprite, iDelay), false, false, false, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 28 -----
    ...
    iDelay.push_back(0);
    vMinion.push_back(
        new Block(29, new Sprite(rR, tSprite, iDelay), true, false, true, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 30 -----
    ...
    iDelay.push_back(120);
    vMinion.push_back(
        new Block(30, new Sprite(rR, tSprite, iDelay), true, false, true, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 31 -----
    ...
    iDelay.push_back(0);
    vMinion.push_back(
        new Block(32, new Sprite(rR, tSprite, iDelay), true, false, true, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 33 -----
    ...
    iDelay.push_back(0);
    vMinion.push_back(
        new Block(34, new Sprite(rR, tSprite, iDelay), true, false, true, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 35 -----
    ...
    iDelay.push_back(0);
    vMinion.push_back(
        new Block(35, new Sprite(rR, tSprite, iDelay), true, false, true, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 36 -----
    tSprite.push_back("vine1");
    ...
    iDelay.push_back(0);
    vMinion.push_back(
        new Block(37, new Sprite(rR, tSprite, iDelay), true, false, true, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 38 -----
    ...
    iDelay.push_back(0);
    vMinion.push_back(
        new Block(39, new Sprite(rR, tSprite, iDelay), true, false, true, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 40 -----
    ...
    iDelay.push_back(0);
    vMinion.push_back(
        new Block(40, new Sprite(rR, tSprite, iDelay), true, false, true, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 41 -----
    ...
    iDelay.push_back(0);
    vMinion.push_back(
        new Block(41, new Sprite(rR, tSprite, iDelay), true, false, true, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 42 -----
    ...
    iDelay.push_back(0);
    vMinion.push_back(
        new Block(42, new Sprite(rR, tSprite, iDelay), true, false, true, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 43 -----
    ...
    iDelay.push_back(175);
    vMinion.push_back(
        new Block(43, new Sprite(rR, tSprite, iDelay), true, false, true, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 44 -----
    ...
    iDelay.push_back(155);
    vMinion.push_back(
        new Block(44, new Sprite(rR, tSprite, iDelay), true, false, true, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 45 -----
    ...
    iDelay.push_back(175);
    vMinion.push_back(
        new Block(45, new Sprite(rR, tSprite, iDelay), true, false, true, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 46 -----
    ...
    iDelay.push_back(155);
    vMinion.push_back(
        new Block(46, new Sprite(rR, tSprite, iDelay), true, false, true, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 47 -----
    ...
    iDelay.push_back(95);
    vMinion.push_back(
        new Block(47, new Sprite(rR, tSprite, iDelay), true, false, true, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 48 -----
    ...
    iDelay.push_back(95);
    vMinion.push_back(
        new Block(48, new Sprite(rR, tSprite, iDelay), true, false, true, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 49 -----
    ...
    iDelay.push_back(0);
    vMinion.push_back(
        new Block(49, new Sprite(rR, tSprite, iDelay), true, false, true, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 50 -----
    ...
    iDelay.push_back(0);
    vMinion.push_back(
        new Block(50, new Sprite(rR, tSprite, iDelay), true, false, true, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 51 -----
    ...
    iDelay.push_back(135);
    vMinion.push_back(
        new Block(51, new Sprite(rR, tSprite, iDelay), true, false, true, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 52 -----
    ...
    iDelay.push_back(75);
    vMinion.push_back(
        new Block(52, new Sprite(rR, tSprite, iDelay), true, false, true, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 53 -----
    ...
    iDelay.push_back(155);
    vMinion.push_back(
        new Block(53, new Sprite(rR, tSprite, iDelay), true, false, true, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 54 -----
    ...
    iDelay.push_back(0);
    vMinion.push_back(
        new Block(54, new Sprite(rR, tSprite, iDelay), true, false, true, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 55 -----
    ...
    iDelay.push_back(0);
    vMinion.push_back(
        new Block(56, new Sprite(rR, tSprite, iDelay), true, false, true, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 57 -----
    ...
    iDelay.push_back(0);
    vMinion.push_back(
        new Block(58, new Sprite(rR, tSprite, iDelay), true, false, true, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 59 -----
    ...
    iDelay.push_back(0);
    vMinion.push_back(
        new Block(59, new Sprite(rR, tSprite, iDelay), true, false, true, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 60 -----
    ...
    iDelay.push_back(0);
    vMinion.push_back(
        new Block(61, new Sprite(rR, tSprite, iDelay), true, false, true, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 62 -----
    tSprite.push_back("fireball_0");
    ...
    iDelay.push_back(155);
    vMinion.push_back(
        new Block(62, new Sprite(rR, tSprite, iDelay), true, false, false, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 63 -----
    tSprite.push_back("firework0");
    ...
    iDelay.push_back(0);
    vMinion.push_back(
        new Block(64, new Sprite(rR, tSprite, iDelay), true, false, false, true));
    tSprite.clear();
    iDelay.clear();
    // ----- 65 -----
    tSprite.push_back("firework1");
...
{
    return "" + std::to_string(1 + currentLevelID / 4) + "-"
           + std::to_string(currentLevelID % 4 + 1);
}
...
void Map::loadMinionsLVL_1_4()
{
    ...
    addFireBall(
        88 * 32, getCFG().GAME_HEIGHT - 16 - 10 * 32, 6, rand() % 360, false);
}
...
void Map::loadMinionsLVL_6_4()
{
    ...
    addFireBall(
        80 * 32, getCFG().GAME_HEIGHT - 16 - 10 * 32, 6, rand() % 360, false);
...
void Map::loadMinionsLVL_8_4()
{
    ...
    addFireBall(
        421 * 32, getCFG().GAME_HEIGHT - 16 - 4 * 32, 6, rand() % 360, false);
    ...
    addFireBall(
        454 * 32, getCFG().GAME_HEIGHT - 16 - 7 * 32, 6, rand() % 360, false);
...
void Map::checkSpawnPoint()
{
    if (getNumOfSpawnPoints() > 1)
    {
        ...
        {
            if (getSpawnPointXPos(i) > oPlayer->getXPos() - fXPos
                && getSpawnPointXPos(i) < oPlayer->getXPos() - fXPos + 128)
            {
                iSpawnPointID = i;
            }
        }
...
void Map::loadPipeEventsLVL_1_1()
{
    lPipe.push_back(new Pipe(
        0, 57, 5, 58, 5, 242 * 32, 32, currentLevelID, 1, false, 350, 1, false));

    lPipe.push_back(new Pipe(1,
                             253,
                             3,
                             253,
                             2,
...
{
    lPipe.push_back(new Pipe(
        0, 103, 4, 104, 4, 242 * 32, 32, currentLevelID, 1, false, 350, 1, false));
    ...
    // -- WARP ZONE 2
    lPipe.push_back(new Pipe(0,
                             186,
                             4,
                             187,
                             4,
                             84,
                             getCFG().GAME_HEIGHT - 80,
                             4,
                             0,
                             true,
                             350,
...
{
    lPipe.push_back(new Pipe(
        0, 103, 5, 104, 5, 232 * 32, 32, currentLevelID, 1, false, 350, 1, false));
...
{
    lPipe.push_back(new Pipe(
        0, 38, 5, 39, 5, 242 * 32, 32, currentLevelID, 1, false, 350, 1, false));
...
{
    lPipe.push_back(new Pipe(
        0, 132, 5, 133, 5, 252 * 32, 32, currentLevelID, 1, false, 350, 1, false));
...
{
    lPipe.push_back(new Pipe(
        0, 84, 4, 85, 4, 272 * 32, 32, currentLevelID, 1, false, 350, 1, false));
...
{
    lPipe.push_back(new Pipe(
        0, 156, 7, 157, 7, 230 * 32, 32, currentLevelID, 1, false, 350, 1, false));
...
{
    lPipe.push_back(
        new Pipe(0, 135, 4, 136, 4, 0, 32, currentLevelID, 2, true, 350, 1, true));
...
{
    lPipe.push_back(
        new Pipe(0, 141, 6, 142, 6, 0, 32, currentLevelID, 2, true, 350, 1, true));
    ...
    lPipe.push_back(new Pipe(
        0, 104, 5, 105, 5, 332 * 32, 32, currentLevelID, 1, false, 350, 1, false));
    ...
    lPipe.push_back(new Pipe(
        0, 238, 4, 239, 4, 362 * 32, 32, currentLevelID, 1, false, 350, 1, false));
...
{
    lPipe.push_back(new Pipe(
        0, 93, 4, 94, 4, 212 * 32, 32, currentLevelID, 1, false, 350, 1, false));
...
{
    lPipe.push_back(new Pipe(
        0, 104, 5, 105, 5, 412 * 32, 32, currentLevelID, 1, false, 350, 1, false));
...
{
    lPipe.push_back(new Pipe(
        0, 156, 5, 157, 5, 242 * 32, 32, currentLevelID, 1, false, 350, 1, false));
...
void Map::loadLVL_2_4()
{
    ...
    vPlatform.push_back(new Platform(3,
                                     1,
                                     85 * 32 + 24,
                                     85 * 32 + 16,
                                     -32,
        ...
        switch (iBlockID)
        {
            ...
            case 55: // ----- BlockQ
                ...
                else
                {
                    ...
                    lMap[nX][nY]->setBlockID(iLevelType == 0 || iLevelType == 4 ? 9
                                             : iLevelType == 1                  ? 56
                                                               : 56);
                }
            ...
            case 81: // ----- Brick
                if (lMap[nX][nY]->getSpawnStar())
                {
                    lMap[nX][nY]->setBlockID(iLevelType == 0 || iLevelType == 4 ? 9
                                             : iLevelType == 1                  ? 56
                                                               : 80);
                    lMinion[getListID(32 * nX)].push_back(new Star(
                ...
                else if (lMap[nX][nY]->getNumOfUse() > 0)
                {
                    ...
                    {
                        lMap[nX][nY]->setBlockID(iLevelType == 0 || iLevelType == 4
                                                     ? 9
                                                 : iLevelType == 1 ? 56
                                                                   : 80);
                    }
...
void Map::EndUse()
{
    ...
    oEvent->vOLDDir.push_back(Mario::Animations::EndPoints);
    oEvent->vOLDLength.push_back(iMapTime);

    oEvent->vOLDDir.push_back(Mario::Animations::Nothing);
    oEvent->vOLDLength.push_back(128);
}
...
void Map::EndBoss()
{
    ...
    if (lMap[getBlockIDX(
            (int) (oPlayer->getXPos() + oPlayer->getHitBoxX() / 2 - fXPos))][6]
            ->getBlockID()
        == 82)
    {
    ...
    oEvent->vOLDDir.push_back(Mario::Animations::BossEnd1);
    oEvent->vOLDLength.push_back(1);
    oEvent->vOLDDir.push_back(Mario::Animations::Nothing);
    oEvent->vOLDLength.push_back(10);

    oEvent->vOLDDir.push_back(Mario::Animations::BossEnd2);
    oEvent->vOLDLength.push_back(1);
    oEvent->vOLDDir.push_back(Mario::Animations::Nothing);
    ...
    for (int i = 0; i < 6; i++)
    {
        oEvent->vOLDDir.push_back(Mario::Animations::BossEnd3);
        oEvent->vOLDLength.push_back(2 + i);
        oEvent->vOLDDir.push_back(Mario::Animations::Nothing);
        oEvent->vOLDLength.push_back(3);
    }

    oEvent->vOLDDir.push_back(Mario::Animations::BossEnd4);
    oEvent->vOLDLength.push_back(1);
    ...
    {
        case 31:
            oEvent->vOLDDir.push_back(Mario::Animations::EndGameBossText1);
            ...
            break;
        default:
            oEvent->vOLDDir.push_back(Mario::Animations::BossText1);
            oEvent->vOLDLength.push_back(
                getBlockIDX(
                    (int) (oPlayer->getXPos() + oPlayer->getHitBoxX() / 2 - fXPos))
                    * 32
    ...
    oEvent->vOLDDir.push_back(Mario::Animations::MarioSprite1);
    oEvent->vOLDLength.push_back(1);
    ...
    {
        case 31:
            oEvent->vOLDDir.push_back(Mario::Animations::EndGameBossText2);
            oEvent->vOLDLength.push_back(
                getBlockIDX(
                    (int) (oPlayer->getXPos() + oPlayer->getHitBoxX() / 2 - fXPos))
                    * 32
            ...
            break;
        default:
            oEvent->vOLDDir.push_back(Mario::Animations::BossText2);
            oEvent->vOLDLength.push_back(
                getBlockIDX(
                    (int) (oPlayer->getXPos() + oPlayer->getHitBoxX() / 2 - fXPos))
                    * 32
...
void Map::playerDeath(bool animation, bool instantDeath)
{
    if ((oPlayer->getPowerLVL() == 0 && !oPlayer->getUnkillAble()) || instantDeath)
    {
        ...
        if (animation)
        {
            ...
            oEvent->vOLDDir.push_back(Mario::Animations::DeathNothing);
            oEvent->vOLDLength.push_back(30);

            oEvent->vOLDDir.push_back(Mario::Animations::DeathTop);
        ...
        {
            oEvent->vOLDDir.push_back(Mario::Animations::LoadingMenu);
            oEvent->vOLDLength.push_back(90);
        ...
        {
            oEvent->vOLDDir.push_back(Mario::Animations::GameOver);
            oEvent->vOLDLength.push_back(90);
...
void Map::startLevelAnimation()
{
    ...
    switch (currentLevelID)
    {
        ...
        case 1:
            ...
            oEvent->vOLDDir.push_back(Mario::Animations::PlayPipeRight);
            oEvent->vOLDLength.push_back(1);
...
void Map::structGND(int X, int Y, int iWidth, int iHeight)
{
    for (int i = 0; i < iWidth; i++)
    {
        ...
        {
            lMap[X + i][Y + j]->setBlockID(iLevelType == 0 || iLevelType == 4 ? 1
                                           : iLevelType == 1                  ? 26
                                           : iLevelType == 2                  ? 92
                                           : iLevelType == 6                  ? 166
                                           : iLevelType == 7                  ? 181
...
void Map::structGND2(int X, int Y, int iSize, bool bDir)
{
    if (bDir)
    {
        for (int i = 0, k = 1; i < iSize; i++)
        {
            for (int j = 0; j < k; j++)
            {
                lMap[X + i][Y + j]->setBlockID(iLevelType == 0 || iLevelType == 4
                                                   ? 25
                                               : iLevelType == 3 ? 167
                                                                 : 27);
            }
    ...
    else
    {
        for (int i = 0, k = 1; i < iSize; i++)
        {
            for (int j = 0; j < k; j++)
            {
                lMap[X + iSize - 1 - i][Y + j]->setBlockID(
                    iLevelType == 0 || iLevelType == 4 ? 25
                    : iLevelType == 3                  ? 167
                                                       : 27);
            }
            ++k;
...
void Map::structGND2(int X, int Y, int iWidth, int iHeight)
{
    for (int i = 0; i < iWidth; i++)
    {
        for (int j = 0; j < iHeight; j++)
        {
            lMap[X + i][Y + j]->setBlockID(iLevelType == 0 || iLevelType == 4 ? 25
                                           : iLevelType == 3                  ? 167
                                                                              : 27);
        }
    }
...
void Map::structPipe(int X, int Y, int iHeight)
{
    ...
    {
        lMap[X][Y + i]->setBlockID(iLevelType == 0   ? 20
                                   : iLevelType == 2 ? 97
                                   : iLevelType == 4 ? 112
                                   : iLevelType == 5 ? 136
                                   : iLevelType == 3 ? 176
                                   : iLevelType == 7 ? 172
                                                     : 30);
        lMap[X + 1][Y + i]->setBlockID(iLevelType == 0   ? 22
                                       : iLevelType == 2 ? 99
                                       : iLevelType == 4 ? 114
                                       : iLevelType == 5 ? 138
                                       : iLevelType == 3 ? 178
    ...
    lMap[X][Y + iHeight]->setBlockID(iLevelType == 0   ? 21
                                     : iLevelType == 2 ? 98
                                     : iLevelType == 4 ? 113
                                     : iLevelType == 5 ? 137
                                     : iLevelType == 3 ? 177
                                     : iLevelType == 7 ? 173
                                                       : 31);
    lMap[X + 1][Y + iHeight]->setBlockID(iLevelType == 0   ? 23
                                         : iLevelType == 2 ? 100
                                         : iLevelType == 4 ? 115
                                         : iLevelType == 5 ? 139
                                         : iLevelType == 3 ? 179
...
{
    lMap[X][Y]->setBlockID(iLevelType == 0   ? 62
                           : iLevelType == 2 ? 105
                           : iLevelType == 4 ? 120
                                             : 38);
    lMap[X][Y + 1]->setBlockID(iLevelType == 0   ? 60
                               : iLevelType == 2 ? 103
                               : iLevelType == 4 ? 118
                                                 : 36);
    ...
    for (int i = 0; i < iWidth; i++)
    {
        lMap[X + 1 + i][Y]->setBlockID(iLevelType == 0   ? 61
                                       : iLevelType == 2 ? 104
                                       : iLevelType == 4 ? 119
                                                         : 37);
        lMap[X + 1 + i][Y + 1]->setBlockID(iLevelType == 0   ? 59
                                           : iLevelType == 2 ? 102
                                           : iLevelType == 4 ? 117
                                                             : 35);
    }

    lMap[X + 1 + iWidth][Y]->setBlockID(iLevelType == 0   ? 58
                                        : iLevelType == 2 ? 101
                                        : iLevelType == 4 ? 116
                                                          : 34);
    lMap[X + 1 + iWidth][Y + 1]->setBlockID(iLevelType == 0   ? 63
                                            : iLevelType == 2 ? 106
                                            : iLevelType == 4 ? 121
                                                              : 39);
}
...
void Map::structBrick(int X, int Y, int iWidth, int iHeight)
{
    for (int i = 0; i < iWidth; i++)
    {
        for (int j = 0; j < iHeight; j++)
        {
            lMap[X + i][Y + j]->setBlockID(iLevelType == 0 || iLevelType == 4 ? 13
                                           : iLevelType == 3                  ? 81
                                                                              : 28);
        }
    }
...
void Map::structCoins(int X, int Y, int iWidth, int iHeight)
{
    for (int i = 0; i < iWidth; i++)
    {
        for (int j = 0; j < iHeight; j++)
        {
            lMap[X + i][Y + j]->setBlockID(iLevelType == 0 || iLevelType == 4 ? 71
                                           : iLevelType == 1                  ? 29
                                           : iLevelType == 2                  ? 73
                                                                              : 29);
        }
...
void Map::structCastleSmall(int X, int Y)
{
    ...
    for (int i = 0; i < 5; i++)
    {
        lMap[X + i][Y + 2]->setBlockID(i == 0 || i == 4  ? iLevelType == 3 ? 157 : 45
                                       : iLevelType == 3 ? 156
                                                         : 44);
    }
