...
Player::Player(SDL_Renderer* rR, float fXPos, float fYPos)
{
    ...
    tempI.push_back(0);
    sMario.push_back(new Sprite(rR, tempS, tempI));
    tempS.clear();
...
void Player::playerPhysics()
{
    if (!CCore::getMap()->getUnderWater())
    {
        ...
        else
        {
            if (onPlatformID == -1)
            {
                onPlatformID = CCore::getMap()->checkCollisionWithPlatform(
                    (int) fXPos, (int) fYPos, getHitBoxX(), getHitBoxY());
                if (onPlatformID >= 0)
                {
                    if (CCore::getMap()->checkCollisionLB(
                            (int) (fXPos - CCore::getMap()->getXPos() + 2),
                            (int) fYPos + 2,
                            getHitBoxY(),
                            true)
                    ...
                    {
                        fYPos = (float) CCore::getMap()
                                    ->getPlatform(onPlatformID)
                                    ->getYPos()
                                - getHitBoxY();
                        resetJump();
            ...
            if (onPlatformID >= 0)
            {
                ...
                else
                {
                    ...
                    //if(moveSpeed == 0)
                    CCore::getMap()->setXPos(
                        CCore::getMap()->getXPos()
                        - CCore::getMap()->getPlatform(onPlatformID)->getMoveX());

                    jumpState = 0;
                }
            }
            else if (!CCore::getMap()->checkCollisionLB(
                         (int) (fXPos - CCore::getMap()->getXPos() + 2),
                         (int) fYPos + 2,
                         getHitBoxY(),
                         true)
    ...
    else
    {
        ...
        {
            CCore::getMap()->addBubble(
                (int) (fXPos - CCore::getMap()->getXPos()
                       + (moveDirection ? getHitBoxX() - rand() % 8 : rand() % 8)),
                (int) fYPos + 4);
...
void Player::movePlayer()
{
    if (bMove && !changeMoveDirection && (!bSquat || powerLVL == 0))
    {
        ...
        else if (SDL_GetTicks() - (100 + 35 * moveSpeed) >= iTimePassed
                 && moveSpeed < currentMaxMove)
        {
            ++moveSpeed;
    ...
    else
    {
        if (SDL_GetTicks()
                    - (50
                       + 15 * (currentMaxMove - moveSpeed)
                             * (bSquat && powerLVL > 0 ? 6 : 1))
                > iTimePassed
            && moveSpeed != 0)
        {
            --moveSpeed;
...
void Player::createFireBall()
{
    if (powerLVL == 2)
    {
        if (nextFireBallFrameID <= 0)
        {
            CCore::getMap()->addPlayerFireBall(
                (int) (fXPos - CCore::getMap()->getXPos()
                       + (moveDirection ? getHitBoxX() : -32)),
...
void Player::updateXPos(int iN)
{
    ...
    if (iN > 0)
    {
        if (!CCore::getMap()->checkCollisionRB(
                (int) (fXPos - CCore::getMap()->getXPos() + iN),
                (int) fYPos - 2,
                getHitBoxX(),
                getHitBoxY(),
                true)
...
void Player::updateYPos(int iN)
{
    ...
    if (iN > 0)
    {
        bLEFT = CCore::getMap()->checkCollisionLB(
            (int) (fXPos - CCore::getMap()->getXPos() + 2),
            (int) fYPos + iN,
            getHitBoxY(),
            true);
        bRIGHT = CCore::getMap()->checkCollisionRB(
            (int) (fXPos - CCore::getMap()->getXPos() - 2),
            (int) fYPos + iN,
            getHitBoxX(),
            getHitBoxY(),
    ...
    {
        bLEFT = CCore::getMap()->checkCollisionLT(
            (int) (fXPos - CCore::getMap()->getXPos() + 2), (int) fYPos + iN, false);
        bRIGHT = CCore::getMap()->checkCollisionRT(
            (int) (fXPos - CCore::getMap()->getXPos() - 2),
            (int) fYPos + iN,
            getHitBoxX(),
            false);

        if (CCore::getMap()->checkCollisionWithPlatform(
                (int) fXPos, (int) fYPos, 0, 0)
                >= 0
            || CCore::getMap()->checkCollisionWithPlatform(
                   (int) fXPos, (int) fYPos, getHitBoxX(), 0)
                   >= 0)
        {
            jumpState = 2;
        }
        ...
        else
        {
            if (jumpState == 1)
            {
                ...
                {
                    Vector2* vRT =
                        getBlockRT(fXPos - CCore::getMap()->getXPos(), fYPos + iN);

                    if (!CCore::getMap()
                             ->getBlock(CCore::getMap()
                                            ->getMapBlock(vRT->getX(), vRT->getY())
                                            ->getBlockID())
                             ->getVisible())
                    {
                        if (CCore::getMap()->blockUse(
                                vRT->getX(),
                                vRT->getY(),
                                CCore::getMap()
                                    ->getMapBlock(vRT->getX(), vRT->getY())
                    ...
                    else if ((int) (fXPos + getHitBoxX()
                                    - CCore::getMap()->getXPos())
                                 % 32
                             <= 8)
                    {
                        updateXPos((int) -(
                            (int) (fXPos + getHitBoxX() - CCore::getMap()->getXPos())
                            % 32));
                    }
                    else if (CCore::getMap()
                                 ->getBlock(
                                     CCore::getMap()
                                         ->getMapBlock(vRT->getX(), vRT->getY())
                                         ->getBlockID())
                ...
                {
                    Vector2* vLT =
                        getBlockLT(fXPos - CCore::getMap()->getXPos(), fYPos + iN);
                    if (!CCore::getMap()
                             ->getBlock(CCore::getMap()
                                            ->getMapBlock(vLT->getX(), vLT->getY())
                                            ->getBlockID())
                             ->getVisible())
                    {
                        if (CCore::getMap()->blockUse(
                ...
                {
                    if ((int) (fXPos + getHitBoxX() - CCore::getMap()->getXPos())
                            % 32
                        > 32 - (int) (fXPos - CCore::getMap()->getXPos()) % 32)
                    {
                        Vector2* vRT = getBlockRT(fXPos - CCore::getMap()->getXPos(),
                                                  fYPos + iN);
                    ...
                    {
                        Vector2* vLT = getBlockLT(fXPos - CCore::getMap()->getXPos(),
                                                  fYPos + iN);
...
bool Player::checkCollisionBot(int nX, int nY)
{
    ...
    if (CCore::getMap()
            ->getBlock(
                CCore::getMap()->getMapBlock(vLT->getX(), vLT->getY())->getBlockID())
            ->getUse())
    {
        CCore::getMap()->blockUse(
            vLT->getX(),
            vLT->getY(),
            CCore::getMap()->getMapBlock(vLT->getX(), vLT->getY())->getBlockID(),
            1);
...
bool Player::checkCollisionCenter(int nX, int nY)
{
    ...
    {
        Vector2* vLT =
            getBlockLC(fXPos - CCore::getMap()->getXPos() + nX, fYPos + nY);
    ...
    {
        Vector2* vLT = getBlockLC(fXPos - CCore::getMap()->getXPos() + nX,
                                  fYPos + nY + (powerLVL > 0 ? 16 : 0));
        ...
        vLT = getBlockRC(fXPos - CCore::getMap()->getXPos() + nX,
                         fYPos + nY + (powerLVL > 0 ? 16 : 0));
        ...
        vLT = getBlockLC(fXPos - CCore::getMap()->getXPos() + nX,
                         fYPos + nY - (powerLVL > 0 ? 16 : 0));
...
{
    return CCore::getMap()->getBlockID((int) nX + getHitBoxX() - 1,
                                       (int) nY + getHitBoxY() + 2);
}
...
int Player::getMarioSpriteID()
{
        ...
        && getCFG().getMM()->getViewID() == Mario::MenuStates::Game)
    {
        ...
        if (unKillAbleTimeFrameID < 90)
        {
            ...
            {
                return powerLVL < 1    ? iSpriteID + 44
                       : powerLVL == 2 ? iSpriteID
                                       : iSpriteID + 11;
            }
            else if (unKillAbleFrameID < 10)
            {
                return powerLVL < 1    ? iSpriteID + 55
                       : powerLVL == 2 ? iSpriteID + 11
                                       : iSpriteID + 22;
            }
            else
            {
